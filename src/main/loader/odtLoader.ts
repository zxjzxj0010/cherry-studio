...
export class OdtLoader extends BaseLoader<{ type: 'OdtLoader' }> {
  ...
  constructor({ filePath, chunkSize, chunkOverlap }: { filePath: string; chunkSize?: number; chunkOverlap?: number }) {
    super(`OdtLoader_${md5(filePath)}`, { filePath }, chunkSize ?? 1000, chunkOverlap ?? 0)
  ...
  override async *getUnfilteredChunks() {
    ...
    console.warn('odt extractedText', this.extractedText)
